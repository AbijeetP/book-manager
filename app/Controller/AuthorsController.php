...
class AuthorsController extends AppController
{
	...
	public function add() {
		if($this->request->is('post')) {
			...
			if($this->Author->Save($this->request->data)) {
				...
				$this->Session->setFlash(__('The author has been saved.'),'default', array('class' => 'success'));
	...
	public function delete($id=null) {
		...
		if($this->Author->hasBooks($id)) {
			$this->Session->setFlash(__('Author has books in the database. Please delete those first.'));
			return $this->redirect(array('action' => 'index'));
		}
		else {
			if($this->Author->delete($id)) {
				$this->Session->setFlash(__('Author was successfully deleted.'),'default', array('class' => 'success'));
				return $this->redirect(array('action' => 'index'));
			}
